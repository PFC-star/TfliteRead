...
absl::Status ConvBuffer1x1::UploadDataForWinograd4x4To6x6(
  ...
  RearrangeWeightsToWinograd4x4To6x6Weights(weights, &wino_weights); // 获得了新的weight
...
absl::Status ConvBuffer1x1::UploadWeights(
  ...
  if (f32_weights) {
    ...
    RearrangeWeightsToOHWIOGroupI4O4(weights, conv_params_.block_size.z,   // 对权值再做一个处理
...
absl::Status ConvBuffer1x1::UploadBiases(
  ...
  int channels = AlignByN(biases.shape.v, 4 * conv_params_.block_size.z); // 128
  // printf("%d\n", channels);
