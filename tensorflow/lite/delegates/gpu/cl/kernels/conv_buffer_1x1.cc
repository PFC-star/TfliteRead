...
absl::Status ConvBuffer1x1::Compile(const CreationContext& creation_context) {
  ...
  RETURN_IF_ERROR(args_.TransformToCLCode(creation_context.device->GetInfo(),
                                          ...
                                          &code));
  // printf("############################ ConvBuffer1x1 Code Start ############################\n");
  // std::cout << code << std::endl;
  // printf("############################# ConvBuffer1x1 Code End #############################\n");
...
absl::Status ConvBuffer1x1::BindArguments() {
  ...
  // printf("####################################################################\n");
  // printf("##################### ConvBuffer 1x1 Start #########################\n");
  // printf("####################################################################\n");
  auto ret = args_.Bind(kernel_.kernel());
  // printf("####################################################################\n");
  // printf("###################### ConvBuffer 1x1 End ##########################\n");
  // printf("####################################################################\n");
  return ret;
}
...
absl::Status CreateConvBuffer1x1Wino4x4To6x6(
  ...
  // printf("%d, %d, %d, %d\n", attr.weights.shape.o, attr.weights.shape.h, attr.weights.shape.w, attr.weights.shape.i);
  ...
  // printf("%d, %d, %d\n", conv_params.block_size.x, conv_params.block_size.y, conv_params.block_size.z);
  // printf("%d\n", conv_params.element_size);
  // printf("%d, %d, %d\n", conv_params.work_group_size.x, conv_params.work_group_size.y, conv_params.work_group_size.z);
